...
$PromptPath = Join-Path $ProjectRoot "claude-stage6a-admin-service-records-prompt.md"
...
Write-Host ""
Write-Host "Starting Claude Code for Stage 6A admin service record management in:" -ForegroundColor Cyan
Write-Host $ProjectRoot
Write-Host ""
Write-Host "File edits will be accepted automatically. Review command approvals carefully." -ForegroundColor Yellow
Write-Host "Approve .\verify-stage5-miniapp.cmd, .\verify-stage6a-admin-service-records.cmd, local build commands, node --check, and read-only git checks." -ForegroundColor Yellow
