...
$PromptPath = Join-Path $ProjectRoot "claude-stage6b3-credential-expiry-prompt.md"
...
Write-Host ""
Write-Host "Starting Claude Code for Stage 6B-3 credential expiry in:" -ForegroundColor Cyan
Write-Host $ProjectRoot
Write-Host ""
Write-Host "File edits will be accepted automatically. Review command approvals carefully." -ForegroundColor Yellow
Write-Host "Approve .\verify-stage6b2-management-status.cmd, .\verify-stage6b3-credential-expiry.cmd, local build commands, local Prisma CLI commands, node --check, and read-only git checks." -ForegroundColor Yellow
